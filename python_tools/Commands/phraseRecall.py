...
def eval(data_original, data_predicted, combined, index):
    with warnings.catch_warnings():
        ...
        if combined:
            ...
            recall_score = metrics.recall_score(data_original['score'], data_predicted['score'], average=None)
            print('{0:8}\t{1:8.5f}\t{2:8.5f}\t{3:8.5f}'.format(index,recall_score[0],recall_score[1],recall_score[2]))
        else:
            recall_score = metrics.recall_score(data_original['score'], data_predicted['score'], average=None)
            print('{0:4}\t{1:4.5f}\t{2:4.5f}\t{3:4.5f}\t{4:4.5f}\t{5:4.5f}'.format(index,recall_score[0],recall_score[1],recall_score[2],recall_score[3],recall_score[4]))
